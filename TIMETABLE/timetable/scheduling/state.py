...
class SchedulerState:
    ...
    teacher_busy: dict[str, dict[str, dict[int, int]]] = field(default_factory=dict)
    room_busy: dict[str, dict[str, dict[int, int]]] = field(default_factory=dict)
    section_busy: dict[str, dict[str, dict[int, int]]] = field(default_factory=dict)
    ...
    def __post_init__(self) -> None:
        self.teacher_busy = {
            teacher.id: {day: defaultdict(int) for day in WEEKDAYS}
            for teacher in self.app_state.teachers
        }
        self.room_busy = {
            room.id: {day: defaultdict(int) for day in WEEKDAYS}
            for room in self.app_state.rooms
        }
        self.section_busy = {
            section.id: {day: defaultdict(int) for day in WEEKDAYS}
    ...
    def resource_free(
        self,
        resource_map: dict[str, dict[str, dict[int, int]]],
        resource_id: str,
        day: str,
        slots: tuple[int, ...],
    ) -> bool:
        return all(resource_map[resource_id][day].get(slot, 0) == 0 for slot in slots)
    ...
    def place_item(self, item: PlacedItem) -> None:
        ...
        for slot in item.slots:
            self.section_busy[item.section_id][item.day][slot] += 1
        for entry in item.entries:
            slots = self.item_slots(entry.start_slot, entry.duration)
            for slot in slots:
                self.teacher_busy[entry.teacher_id][entry.day][slot] += 1
                self.room_busy[entry.room_id][entry.day][slot] += 1
    ...
    def remove_item(self, item_id: str) -> PlacedItem:
        ...
        for slot in item.slots:
            self.section_busy[item.section_id][item.day][slot] -= 1
            if self.section_busy[item.section_id][item.day][slot] <= 0:
                del self.section_busy[item.section_id][item.day][slot]
        for entry in item.entries:
            slots = self.item_slots(entry.start_slot, entry.duration)
            for slot in slots:
                self.teacher_busy[entry.teacher_id][entry.day][slot] -= 1
                self.room_busy[entry.room_id][entry.day][slot] -= 1
                if self.teacher_busy[entry.teacher_id][entry.day][slot] <= 0:
                    del self.teacher_busy[entry.teacher_id][entry.day][slot]
                if self.room_busy[entry.room_id][entry.day][slot] <= 0:
                    del self.room_busy[entry.room_id][entry.day][slot]
            self.teacher_loads[entry.teacher_id] -= entry.duration
            self.teacher_day_loads[entry.teacher_id][entry.day] -= entry.duration
            self.teacher_section_subject_counts[entry.teacher_id][entry.section_id][entry.subject_id] -= 1
