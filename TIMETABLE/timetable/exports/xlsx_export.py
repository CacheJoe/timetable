...
def room_workbook_bytes(app_state) -> bytes:
    tables = build_room_tables(app_state)
    sheets = [(name, rows_to_matrix(rows)) for name, rows in tables.items()]
    sheets.append(("Room Occupancy", rows_to_matrix(build_room_occupancy_rows(app_state))))
    return workbook_bytes(sheets)
